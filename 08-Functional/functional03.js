 ...
 * 
 * But before we get there, let's look at an imperative approach to programming to highlight where you 
 * may have issues.
 * 
 * In English (and many other languages), the imperative tense is used to give commands. 
 * 
 * Similarly, an imperative style in programming is one that gives the computer a set of statements to 
 * perform a task.
 * 
 * Often the statements change the state of the program, like updating global variables. 
 * 
 * A classic example is writing a for loop that gives exact directions to iterate over the indices of 
 * an array.
 * 
 ...
 * 
 * JavaScript offers many predefined methods that handle common tasks so you don't need to write 
 * out how the computer should perform them. 
 * 
 * For example, instead of using the for loop mentioned above, you could call the map method which 
 * handles the details of iterating over an array. 
 * 
 * This helps to avoid semantic errors, like the "Off By One Errors" that were covered in the 
 * Debugging section.
 * 
 ...
 * It's using a method that has side effects in the program, causing incorrect output. 
 * 
 ...
 * 
 * Work through the code and see if you can figure out the problem, then advance to the next challenge to 
 * learn more.
 */
...
// Problem in the code above - 'Vine' is also not included from the videoWindow array