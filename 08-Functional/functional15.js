 ...
 * 
 * For example, if you want to compare elements of the array, or add them together, filter or map could not 
 * process that.
 * 
...
var averageRating = watchList.filter((x) => x.Director === "Christopher Nolan").map((x) => Number(x.imdbRating)).reduce((x, y) => x + y) / watchList.filter((x) => x.Director === "Christopher Nolan").length;
